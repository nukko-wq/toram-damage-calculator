...
### Grazeダメージ計算式
Graze状態では、Criticalダメージと同じ計算過程を経て、安定率のみ半減して計算します。
...
**Graze安定率の計算**:
- 基本安定率を半減（`Math.floor(基本安定率 / 2)`）して最小値とする
- 最大値は100%のまま
...
3. **graze（グレイズ）**:
   - クリティカルダメージと同じ計算過程（baseDamageはクリティカル倍率適用済み）
   - 安定率を半減: `Math.floor(基本安定率 / 2)` で最小安定率を計算
   - 最大安定率は100%のまま、平均安定率は `Math.floor((最小 + 最大) / 2)`
