...
- **属性条件**: 
  - **確定適用**: 属性攻撃が「有(有利)」の場合
  - **仕様変更**: 「有(その他)」「無」「不利属性」の場合の適用可否は調査中（2025年アップデート後）
...
- **属性攻撃**: 
  - **確定適用**: 有利属性攻撃時
  - **調査中**: その他・無属性・不利属性での適用可否（2025年アップデート後の仕様変更）
...
- **属性制限**: 
  - **確定**: 有利属性攻撃時は適用
  - **調査中**: その他・無属性・不利属性での適用可否（2025年アップデート後）
...
**計算手順:**
1. 属性条件判定 = その他属性攻撃 → **調査中**（2025年アップデート後の仕様変更により）
2. 基礎INT補正 = **TBD**（適用可否調査中）
3. 総属性有利 = **TBD**
4. **最終属性有利補正: TBD**
...
**計算手順:**
1. 属性条件判定 = 無属性攻撃 → **調査中**（2025年アップデート後の仕様変更により）
2. 基礎INT補正 = **TBD**（適用可否調査中）
3. 総属性有利 = **TBD**
4. **最終属性有利補正: TBD**
...
**計算手順:**
1. 属性条件判定 = 不利属性攻撃 → **調査中**（2025年アップデート後の仕様変更により）
2. 基礎INT補正 = **TBD**（適用可否調査中）
3. 総属性有利 = **TBD**
4. **最終属性有利補正: TBD**
...
function calculateINTElementAdvantage(
  ...
  switch (elementAttack) {
    ...
    case 'disadvantageous':
      isElementApplicable = false // 調査中のため一旦false
      note = '2025年アップデート後の仕様変更により適用可否調査中'
      break
    default:
...
1. **仕様調査**: 「有(その他)」「無」「不利属性」での基礎INT補正の適用可否
2. **適用タイミング**: 他の補正との計算順序
3. **特殊条件**: 特定の条件下での動作変更の有無
4. **アップデート詳細**: 2025年アップデートでの正確な変更内容