...
4. **総ステータスポイント = 172 + 0 + 0 = 172**

#### 例2: レベル10の場合
**計算手順:**
1. 基本ポイント = 172
2. レベル増加ポイント = (10 - 1) × 2 = 18
3. レベル305ボーナス = 0（レベル305未満）
4. **総ステータスポイント = 172 + 18 + 0 = 190**

#### 例3: レベル150の場合
**計算手順:**
1. 基本ポイント = 172
2. レベル増加ポイント = (150 - 1) × 2 = 298
3. レベル305ボーナス = 0（レベル305未満）
4. **総ステータスポイント = 172 + 298 + 0 = 470**

#### 例4: レベル300の場合
**計算手順:**
1. 基本ポイント = 172
2. レベル増加ポイント = (300 - 1) × 2 = 598
3. レベル305ボーナス = 0（レベル305未満）
...
```typescript
interface StatPointCalculationSteps {
  level: number
  basePoints: number
...
**重要な前提条件**: 物理のクリティカルダメージ計算で300以降上昇率半減の効果を受けた後に魔法クリティカルダメージが計算される。

### 詳細計算

#### 構成要素
- **クリティカルダメージ**: 物理クリティカルダメージの最終計算結果（300超過時の半減処理適用済み）
- **スペルバーストslv**: バフスキルのスペルバースト(sf1)のスキルレベル（0-10）
- **基本値100**: 魔法クリティカルダメージのベース値

#### 計算手順
1. **物理クリティカルダメージ取得**: 300超過時半減処理を含む最終値を使用
2. **スペルバーストレベル確認**: バフスキルからスペルバーストのスキルレベルを取得
3. **増加量計算**: (クリティカルダメージ - 100) × (20 + スペルバーストslv) / 40
4. **最終値算出**: INT(100 + 増加量)
...
#### 例1: 標準的なケース
**入力値:**
- 物理クリティカルダメージ: 250（半減処理前）
- スペルバーストlv: 5

**計算手順:**
1. 物理クリティカルダメージ = 250（300以下のため半減処理なし）
2. 増加量 = (250 - 100) × (20 + 5) / 40 = 150 × 25 / 40 = 93.75
3. 魔法クリティカルダメージ = INT(100 + 93.75) = INT(193.75) = 193

#### 例2: 高クリティカルダメージケース（半減適用）
**入力値:**
- 物理クリティカルダメージ（半減処理前）: 420
- 物理クリティカルダメージ（半減処理後）: 300 + INT((420-300)/2) = 300 + 60 = 360
- スペルバーストlv: 8

**計算手順:**
1. 物理クリティカルダメージ = 360（半減処理適用済み）
2. 増加量 = (360 - 100) × (20 + 8) / 40 = 260 × 28 / 40 = 182
3. 魔法クリティカルダメージ = INT(100 + 182) = INT(282) = 282
...
**計算手順:**
1. 物理クリティカルダメージ = 180（300以下のため半減処理なし）
2. 増加量 = (180 - 100) × (20 + 3) / 40 = 80 × 23 / 40 = 46
3. 魔法クリティカルダメージ = INT(100 + 46) = INT(146) = 146
...
**計算手順:**
1. 物理クリティカルダメージ = 300（300ちょうどのため半減処理なし）
2. 増加量 = (300 - 100) × (20 + 0) / 40 = 200 × 20 / 40 = 100
3. 魔法クリティカルダメージ = INT(100 + 100) = INT(200) = 200

#### 例5: 最大レベルスペルバーストケース
**入力値:**
- 物理クリティカルダメージ（半減処理前）: 500
- 物理クリティカルダメージ（半減処理後）: 300 + INT((500-300)/2) = 300 + 100 = 400
- スペルバーストlv: 10

**計算手順:**
1. 物理クリティカルダメージ = 400（半減処理適用済み）
2. 増加量 = (400 - 100) × (20 + 10) / 40 = 300 × 30 / 40 = 225
3. 魔法クリティカルダメージ = INT(100 + 225) = INT(325) = 325

### 構成要素の詳細

**スペルバーストスキルレベルの効果:**
- Lv0（未習得）: 係数 20/40 = 0.5倍
- Lv5（中級）: 係数 25/40 = 0.625倍
- Lv10（最大）: 係数 30/40 = 0.75倍

**物理クリティカルダメージとの関係:**
- 物理クリティカルダメージが高いほど、魔法クリティカルダメージも高くなる
- ただし、物理側の300超過半減の影響を受ける
- スペルバーストレベルによって増加率が変わる

### TypeScript実装例

```typescript
interface MagicalCriticalDamageCalculationSteps {
  physicalCriticalDamage: number
  spellBurstLevel: number
  baseMagicalCD: number
  increaseAmount: number
  finalMagicalCriticalDamage: number
}

function calculateMagicalCriticalDamage(
  physicalCriticalDamage: number,
...
// 統合計算例（物理クリティカルダメージから魔法クリティカルダメージまで）
function calculateBothCriticalDamages(
  adjustedSTR: number,
  adjustedAGI: number,
  spellBurstLevel: number,
  allBonuses: AllBonuses = {}
): {
  physical: CriticalDamageCalculationSteps
  magical: MagicalCriticalDamageCalculationSteps
} {
  // 1. 物理クリティカルダメージ計算
  const physicalCD = calculateCriticalDamage(adjustedSTR, adjustedAGI, allBonuses)
  
  // 2. 魔法クリティカルダメージ計算（物理の最終値を使用）
  const magicalCD = calculateMagicalCriticalDamage(
...
1. **物理CD依存**: 魔法クリティカルダメージは物理クリティカルダメージの最終値に依存
2. **半減処理後使用**: 物理側の300超過半減処理が適用された後の値を使用
3. **スペルバーストスキル依存**: バフスキルのスペルバーストレベルが必須
4. **INT()関数**: 最終計算時に小数点以下切り捨て
5. **魔法攻撃専用**: 物理攻撃には適用されない
...
#### 構成要素
- **ATK**: メイン武器のATK計算結果（物理攻撃力）
- **サブATK**: サブ武器のATK計算結果（双剣専用）

#### 計算条件
- **双剣装備時**: メインATKとサブATKを加算
- **その他武器種**: メインATKのみを使用
- **サブ武器なし**: 双剣でもサブATKは0として扱う
...
**計算手順:**
1. 双剣のため、総ATK = ATK + サブATK
2. 総ATK = 2,200 + 350 = 2,550
3. **総ATK = 2,550**
...
**計算手順:**
1. 双剣のため、総ATK = ATK + サブATK
2. 総ATK = 2,200 + 0 = 2,200
3. **総ATK = 2,200**

### TypeScript実装例

```typescript
interface TotalATKCalculationSteps {
  mainATK: number
  subATK: number
  isDualSword: boolean
  totalATK: number
...
1. **武器種判定**: メイン武器種が「双剣」の場合のみサブATKを加算
2. **サブ武器依存**: 双剣でもサブ武器が設定されていない場合、サブATKは0
3. **単純加算**: 双剣時は単純にATK + サブATKで計算
4. **表示項目**: StatusPreviewの基本ステータス「総ATK」項目に表示
5. **ダメージ計算**: 最終的なダメージ計算で使用される主要な攻撃力値
...
| 2025-07-01 | 魔法クリティカルダメージ計算式を追加 | スペルバーストスキル依存、物理CD後の計算、INT関数使用、5つの計算例とTypeScript実装例を含む |
| 2025-07-02 | 総ATK計算式を追加 | 双剣専用のATK+サブATK加算、3つの計算例とTypeScript実装例を含む |
