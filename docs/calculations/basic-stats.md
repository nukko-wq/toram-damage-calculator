...
MATK（魔法攻撃力）の計算については、専用ドキュメントを参照してください。

**詳細**: [MATK計算式](./matk-calculation.md)

**概要**:
- **基本計算式**: `MATK = INT((自Lv+総武器MATK+ステータスMATK+MATKアップ-MATKダウン)×(1+MATK%/100))+MATK固定値`
- **武器種別依存**: 総武器MATKとステータスMATKは武器種により計算式が大きく異なる
- **2つのステータス**: MATKアップ・ダウンは基礎ステータス、ステータスMATKは補正後ステータスを使用
- **INT()関数**: %適用時に小数点以下切り捨て処理を実施
