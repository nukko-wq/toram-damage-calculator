...
#### 2.1.1 新規装備作成フロー
```
装備タブ（例：メイン装備） → [新規登録] ボタン → 装備名入力モーダル → 既存の入力フォーム画面
```
...
5. モーダルが閉じ、既存の装備入力フォーム画面に遷移
6. 現在のタブに応じた装備カテゴリで新規装備が作成状態になる
...
interface PropertyEntry {
  key: keyof EquipmentProperties  // プロパティ名
  value: number                   // 値
  isPercentage: boolean          // %系かどうか
  min: number                    // 最小値
  max: number                    // 最大値
}
```
...
interface EquipmentEditorState {
  ...
  originalEquipment?: PresetEquipment
  formData: {
    name: string
    source: string
...
export function saveUserEquipment(equipment: UserEquipment): Promise<void>
