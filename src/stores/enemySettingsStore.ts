...
import type { EnemySettings, EnemySettingsMap, BossDifficulty } from '@/types/calculator'
...
interface EnemySettingsStore {
	...
	updateEnemySettings: (enemyId: string, updates: Partial<Omit<EnemySettings, 'enemyId' | 'lastUpdated'>>) => void
	setBossDifficulty: (enemyId: string, difficulty: BossDifficulty) => void
	setRaidBossLevel: (enemyId: string, level: number) => void
	setManualOverrides: (enemyId: string, overrides: { resistCritical?: number; requiredHIT?: number }) => void
	...
	devtools(
		(set, get) => ({
			...
			updateEnemySettings: (enemyId: string, updates) => {
				...
				}, false, `updateEnemySettings:${enemyId}`)
			},
			...
			loadFromLocalStorage: () => {
				try {
					...
					console.warn('Failed to load enemy settings from localStorage:', error)
				}
			},
