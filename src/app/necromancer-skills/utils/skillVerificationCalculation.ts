...
export function calculateSkillParameters(
	...
	for (
		let testMultiplier = Math.max(50, initialMultiplier - 100);
		testMultiplier <= initialMultiplier + 100;
		testMultiplier += 1
	) {
		// 固定値を逆算
		const estimatedFixed =
			(input.baseDamage * 100) / testMultiplier - baseValue1

		// 固定値の候補を推定値周辺で探索
		for (
			let testFixed = Math.max(-50, Math.floor(estimatedFixed) - 20);
			testFixed <= Math.floor(estimatedFixed) + 20;
			testFixed++
		) {
			// 検証：この倍率と固定値で計算した結果
			const verifyDamage1 = Math.floor(
				((baseValue1 + testFixed) * testMultiplier) / 100,
			)
			const verifyDamage2 = Math.floor(
				((baseValue2 + testFixed) * testMultiplier) / 100,
			)
	...
	if (bestResult.error > 0) {
		...
		for (
			let testMultiplier = roughMultiplier - 2;
			testMultiplier <= roughMultiplier + 2;
			testMultiplier += 0.1
		) {
			for (
				let testFixed = roughFixed - 5;
				testFixed <= roughFixed + 5;
				testFixed++
			) {
				// 検証：この倍率と固定値で計算した結果
				const verifyDamage1 = Math.floor(
	...
	const step4_afterSkill = Math.floor(
		(step3_afterElement * skillMultiplier) / 100,
	) // INT関数はここで適用
