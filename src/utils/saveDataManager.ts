...
export async function ensureDefaultSaveData(): Promise<DefaultSaveData> {
	...
	const defaultData = saveDataList.find(save => save.id === 'default') as DefaultSaveData
 ...
 * 新しいセーブデータを作成
 */
export async function createSaveData(name: string, currentData: CalculatorData): Promise<SaveData> {
	const saveDataList = StorageHelper.get<SaveData[]>(STORAGE_KEYS.SAVE_DATA_LIST, [])
	const now = new Date().toISOString()
 ...
 * セーブデータを更新
 */
export async function updateSaveData(id: string, updates: Partial<SaveData>): Promise<void> {
	const saveDataList = StorageHelper.get<SaveData[]>(STORAGE_KEYS.SAVE_DATA_LIST, [])
 ...
 * セーブデータ名を変更
 */
export async function renameSaveData(id: string, newName: string): Promise<void> {
	if (id === 'default') {
...
export async function reorderSaveData(newOrder: string[]): Promise<void> {
	...
	const missingItems = saveDataList.filter(save => !newOrder.includes(save.id))
	reorderedList.push(...missingItems.map((item, index) => ({ 
		...item, 
...
export function getCurrentSaveData(): SaveData {
	const currentSaveId = StorageHelper.get(STORAGE_KEYS.CURRENT_SAVE_ID, 'default')
	const saveDataList = StorageHelper.get<SaveData[]>(STORAGE_KEYS.SAVE_DATA_LIST, [])
	
	const currentSave = saveDataList.find(save => save.id === currentSaveId)
	if (!currentSave) {
		// 現在のセーブデータが見つからない場合、デフォルトを返す
		return saveDataList.find(save => save.id === 'default') || createDefaultSaveData()
	}
...
async function validateStorageIntegrity(): Promise<boolean> {
	try {
		...
		// 現在のセーブIDが有効かチェック
		if (currentSaveId && !saveDataList.some(save => save.id === currentSaveId)) {
			return false
		}
