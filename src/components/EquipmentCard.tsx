...
export default function EquipmentCard({ equipment, isSelected, onClick }: EquipmentCardProps) {
	...
	const formatProperties = () => {
		const props = Object.entries(equipment.properties)
			...
			.map(([key, value]) => `${key}${value > 0 ? '+' : ''}${value}${key.includes('%') ? '' : ''}`)
			.slice(0, 3) // 最大3つまで表示
	...
	return (
		...
		>
			...
			{/* タイプバッジ */}
			<div className="mb-2">
				<span className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${getTypeColor(equipment.type)}`}>
					{getTypeLabel(equipment.type)}
				</span>
			</div>

			{/* 装備名 */}
			<h3 className="font-semibold text-gray-900 mb-2 pr-8">{equipment.name}</h3>
