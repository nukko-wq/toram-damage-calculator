...
export function useEquipmentDamageSorting(
	...
	const [equipmentsWithDamage, setEquipmentsWithDamage] = useState<EquipmentWithDamage[]>([])
	...
	useEffect(() => {
			...
			try {
				...
				const results = await Promise.all(
					equipments.map(async (equipment) => {
						try {
							// シミュレーション後のダメージを計算
							const simulatedData = simulateItemEquipSimple(currentData, equipment, slotInfo)
		...
		calculateDamages()
	}, [equipments, slotInfo, enabled, currentData, currentResults, powerOptions, baselineDamageResult])
