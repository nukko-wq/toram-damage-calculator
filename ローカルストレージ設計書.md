...
**具体的な動作例**:
```
1. カスタム武器（ATK=5で保存済み）を選択
2. ATK=10に変更 → 編集セッションにのみ保存（永続データはATK=5のまま）
3. セーブ実行 → 永続データをATK=10に更新
4. リロード/切り替え（セーブなし） → 編集セッション削除、永続データ（ATK=5）を復元
```
...
export function updateEquipmentProperties(id: string, properties: Partial<EquipmentProperties>): Promise<void>  // プロパティ連動更新
