...
**初期化フロー**:
```
アプリ起動 → ストレージチェック → メインデータ存在確認 → [存在しない場合]メインデータ作成 → プリセットデータをローカルストレージにコピー → メインデータを現在のセーブデータに設定 → アプリ開始
```
...
4. **プリセットデータのローカルストレージコピー**:
   - `src/data/equipments.json` → LocalStorage (`preset_equipments`)
   - `src/data/crystals.json` → LocalStorage (`preset_crystals`)
   - `src/data/enemies.json` → LocalStorage (`preset_enemies`)
...
async function initializePresetData(): Promise<void> {
  // 既にコピー済みかチェック
  const equipmentsExist = localStorage.getItem(STORAGE_KEYS.PRESET_EQUIPMENTS)
  const crystalsExist = localStorage.getItem(STORAGE_KEYS.PRESET_CRYSTALS)
  const enemiesExist = localStorage.getItem(STORAGE_KEYS.PRESET_ENEMIES)
