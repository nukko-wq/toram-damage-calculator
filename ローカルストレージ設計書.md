...
**データ交換**（既存のcrystalDatabase.ts準拠）:
```typescript
// 既存の関数を拡張
export function exportUserCrystals(): string
export function importUserCrystals(jsonData: string): boolean

// 個別エクスポート機能を追加
export function exportUserCrystal(id: string): string
```
...
```json
{
  ...
  "crystals": [
    {
      "id": "user_custom_crystal_001",
      "name": "カスタム火力クリスタル",
      "type": "weapon",
      "properties": {
        "ATK_Rate": 15,
        "CriticalDamage_Rate": 12,
        "PhysicalPenetration_Rate": 8
      },
      "description": "火力特化のカスタムクリスタル",
      "isCustom": true,
      "createdAt": "2025-01-15T14:22:33.000Z",
      "updatedAt": "2025-01-16T09:45:12.000Z",
      "isFavorite": true
    }
  ]
}
...
**自動保存対応**:
- フォーム値変更から3秒後にデバウンス自動保存
- 現在選択中のセーブデータに保存
- ページ離脱時（beforeunload）の保存
