...
**UI動作仕様**
- 新規作成クリック → 装備名入力モーダル表示 → 入力確定でカスタム装備作成 → 自動的に装備スロットにセット
- 削除クリック → 削除確認モーダル表示 → 確定でカスタム装備を削除
- プリセット選択モーダルでカスタム装備も選択可能（統合表示）
- カスタム装備は視覚的に区別表示（アイコン、ラベル等）

**カスタム装備保存管理システム**
- 仮データ管理：新規作成時はメモリ上に一時装備データを作成（未保存状態）
- セーブ時永続化：「現在のデータを保存」実行時のみLocalStorageに永続保存
- 未保存データ削除：セーブデータ切り替え・ブラウザリロード時の自動クリーンアップ
- 保存状態表示：未保存カスタム装備の視覚的インジケーター

**カスタム装備プロパティ連動システム**
- プロパティフォーム変更検知：カスタム装備セット中のプロパティ入力変更を検知
- リアルタイム反映：プロパティ変更をカスタム装備の実データに即座に同期
...
interface CustomEquipmentState {
  isTemporary: boolean        // 仮データかどうか
  isModified: boolean         // 保存後に変更されたかどうか
...
interface PropertySyncConfig {
  enableAutoSync: boolean     // 自動同期の有効/無効
  syncTarget: string          // 同期対象の装備ID
...
**カスタム装備データフロー**
```
新規作成 → 仮データ作成（メモリ） → プロパティ編集 → リアルタイム反映
                                              ↓
セーブデータ保存 → LocalStorage永続化 ← 現在のデータを保存

セーブデータ切り替え/リロード → 仮データ削除 → 初期状態に復帰
```

**プロパティ連動フロー**
```
プロパティフォーム変更 → カスタム装備検知 → 装備データ更新 → 未保存状態設定
                                    ↓
                    装備選択モーダル → 更新された装備表示
```
