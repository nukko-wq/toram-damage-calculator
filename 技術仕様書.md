...
```typescript
// 名前変更・削除ボタンの表示判定
const showCustomEquipmentButtons = (
...
// 名前変更ボタンの表示
const showRenameButton = showCustomEquipmentButtons

// 削除ボタンの表示
const showDeleteButton = showCustomEquipmentButtons
```

**UI動作仕様**
- 新規作成クリック → 装備名入力モーダル表示 → 入力確定でカスタム装備作成 → 自動的に装備スロットにセット
- 名前変更クリック → 名前変更モーダル表示 → 現在名を初期値として設定 → 新しい名前入力 → 確定で名前更新（カスタム装備のみ）
- 削除クリック → 削除確認モーダル表示 → 確定でカスタム装備を削除（カスタム装備のみ）
