...
interface PresetEquipment {
  id: string                    // 一意識別子
  name: string                  // 装備名
  itemType: string              // 装備種類（片手剣、重装備等）
  baseStats: BaseEquipmentStats // 基本性能
  properties: Partial<EquipmentProperties> // 付与プロパティ
  description?: string          // 説明文
  source?: string              // 入手方法
  rarity?: EquipmentRarity     // レアリティ
  level?: number               // 装備レベル
}
```
...
interface BaseEquipmentStats {
  ATK: number         // 攻撃力 (0-1500)
  DEF: number         // 防御力 (0-1500)
  MATK: number        // 魔法攻撃力 (0-1500)
  MDEF: number        // 魔法防御力 (0-1500)
...
```typescript
type EquipmentRarity = 'common' | 'rare' | 'epic' | 'legendary' | 'mythic'
```
